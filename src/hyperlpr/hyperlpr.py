...
import numpy as np
import os
from .table_chs import chars, chars_latin
...
class LPR:
    def __init__(self, folder):
        ...
        modelRecognitionPath = [
            os.path.join(folder, "dnn/SegmenationFree-Inception.prototxt"),
            os.path.join(folder, "dnn/SegmenationFree-Inception.caffemodel"),
        ]
        modelFineMappingPath = [
            os.path.join(folder, "dnn/HorizonalFinemapping.prototxt"),
            os.path.join(folder, "dnn/HorizonalFinemapping.caffemodel"),
        ]
        mini_ssd_path = [
            os.path.join(folder, "dnn/mininet_ssd_v1.prototxt"),
            os.path.join(folder, "dnn/mininet_ssd_v1.caffemodel"),
        ]
        refine_net_path = [
            os.path.join(folder, "dnn/refinenet.prototxt"),
    ...
    def run(self, image, minSize=30, charSelectionDeskew=True, mode="ssd"):
        """
    ...
    def detect_ssd(self, im):
        ...
        for i in range(3):
            im_tensor[0, i, :, :] = (
                im[:, :, 2 - i] / pixel_scale - pixel_means[2 - i]
            ) / pixel_stds[2 - i]
        self.ssd_detection.setInput(im_tensor)
        # print(f"Tensor shape is {im_tensor.shape}")
        cropped_images = []
    ...
    def detect_traditional(self, image_gray, resize_h=720, en_scale=1.1, minSize=30):
        ...
        :return:   the results.
        """
        watches = self.detector.detectMultiScale(
            image_gray, en_scale, 3, minSize=(minSize * 4, minSize)
        )
        cropped_images = []
    ...
    def to_refine(self, image, pts, scale=3.0):
        ...
        ty4 = cy + ch // 2
        target_pts = (
            np.array([[tx1, ty1], [tx2, ty2], [tx3, ty3], [tx4, ty4]]).astype(
                np.float32
            )
            * scale
        )
        ...
        mat_, inliers = cv2.estimateAffine2D(org_pts, target_pts)
    ...
    def finetune(self, image_, stage=2):
        ...
        h, w, c = image_.shape
        pts = (
            self.refine_net.forward("conv6-3").reshape(4, 2) * np.array([w, h])
        ).astype(int)
    ...
    def segmentation_free_recognition(self, src):
        ...
        blob = cv2.dnn.blobFromImage(
            temp, 1 / 255.0, (40, 160), (0, 0, 0), False, False
        )
